...
正因为射程如此，agent 对自身能力的自述必须是**测量**而不是散文：`Sandbox.reach()` 返回结构化事实，`reach(probe=True)` 跑一次真实往返（在 cwd 之外写读一个文件 + 解析域名）作为证据；`my_capabilities` 直接渲染它，`forge_tool` 与 `my_capabilities` 的工具描述里也写明 —— 描述每回合都随请求下发，比提示词里的一段话更难被忽略。「我没有文件工具」是错的：锻造一个就有。

**同一个病还有另外两半，一起治了。**「无持久记忆」也是假的：账本是磁盘上的 sqlite（`store.report()` 报路径、工具数、事件数、存活时长），每次 forge／amendment／spawn／run 都追加进去，重启不丢 —— 而锻造出的工具此前**根本不落盘**（`forge_tool` 只记事件、只有 `evolve_tool` 会 `save_tool`），所以那句"每次调用是独立快照"曾经半真；现在落盘了。「无连续自我」同样是假的：`my_history` 直接读账本和 selfmod 日志，"我做过什么／谁改了我"有据可查。最后，`_self_report()` 把这三样（射程／记忆／自身源码路径）在**每次请求**里重算并注入提示词，所以自述无法再漂移。→ 取 Codex 的进程隔离精华，扔它跨平台不一致的糟粕（Windows 用 timeout 兜底）。
