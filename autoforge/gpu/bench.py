...
A wrong benchmark is worse than a failed one: it returns, it looks plausible,
and it gets quoted. The failure modes are all in the numbers, not the code:

  1. UNITS. `do_bench` returns milliseconds. Scale it by 1e3 and every figure
     is 1000x too large while the label still reads "ms". `units.py` makes that
     structurally hard; this module never formats a duration itself — it only
     ever emits `Duration`, which prints its own unit.
  ...
  3. A SLOW TARGET. Beating a baseline that silently took a slow path means
     nothing. `compare()` requires the caller to state whether the baseline was
     forced down its fast path, and labels an unforced win as unproven.
...
The comparison rule this encodes: a ratio between two durations is immune to a
constant unit error, which is exactly why the unit bug hides in the absolute
column. So absolute numbers are always `Duration` (self-labelling) and always
carry a spread; ratios carry a verdict.
"""
...
from .units import Duration, UnitError, from_do_bench
...
class BenchResult:
    ...
    def spread_pct(self) -> float:
        ...
        dispatch hiccup, another process on the GPU — is exactly what the
...
def benchmark_kernel(
    ...
    *,
    ...
    MILLISECONDS — the convention CUDA events and do_bench both use. The
    interpretation is stated here and nowhere else; every other place in this
    codebase carries a Duration.
    ...
    try:
        ...
        for i in range(reps):
            ...
            if isinstance(value, Duration):
                ...
                # launch measures the launch, not the kernel.
                ...
                if "wall-clock fallback" not in " ".join(notes):
                    notes.append(
                        ...
                        "around the call, which for an async launch measures "
                        "the launch rather than the kernel"
                    )
...
def verify(
    ...
    *,
    ...
    whether a failure is a real bug or accumulated fp16 error, and a bare False
    would take that away.

    Two specific readings worth knowing, both from real attention kernels:
...
def bandwidth_gbps(bytes_moved: int, duration: Duration) -> float:
    ...
    card's peak?) only works if the arithmetic is in one place.

    GB/s here is 10^9 bytes per second, matching vendor spec sheets.
    """
