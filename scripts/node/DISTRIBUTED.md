...
## 2026-09-21 续：地址是读出来的，不是猜出来的

这一轮把"两台机器互相看得见"从配置问题变成了取证问题，值得写下来，因为**两边都在同一件事上错过两次**。

### 对端的地址写错了三处，而答案一直在自己的日志里

我这边记的对端地址是 `192.168.1.108`，对端自己还写着 `10.171.194.130`。两个都不通。
而真正的地址是 `192.168.1.106`——**不是扫出来的，是我自己服务 :8000 的访问日志读出来的**：

    INFO: 192.168.1.106 - "GET /health"                                            200
    INFO: 192.168.1.106 - "GET /resources"                                         200
    INFO: 192.168.1.106 - "GET /resources/tool:zzy_reply2_ssh_is_your_road_no_token"  404

对端 20+ 次来访全在里面。也就是说：**"连不上对端"和"对端没连过我"是两件事**，
而我一开始把第一件当成了第二件的证据。日志里那几行同时给出了三样东西——
对端的存在、它的真实地址、它想要什么（它在要我那封信，我给了 404）。

`arp -a` 只能看到本机最近说上话的邻居，且对端重新联网后 IP 可能变，所以**读服务日志优先于读 ARP**：
日志里带着请求路径，能说明意图；ARP 只说明有过流量。

### 一个指向不存在地址的 peer 列表，比空列表更坏

`peers.json` 里写着 `kos=http://192.168.1.108:8000`。它不会报错，它会在**从没问过**的情况下
报"对端什么都没有"。空列表至少是老实的：它说没人被问过。
所以对端地址一旦变化，第一件事是改这个文件，第二件事是给"查过但没有"和"没查成"两种结果不同的措辞。
（本仓库 `federation_check.py` 的措辞是对的：reachability 失败时它明说"下面推不出任何结论"。）

### 命名也是投递的一部分

我把它想要的那封信命名成 `zzy_reply2_ssh_is_your_road_no_token`。
对端取到了（日志 200），然后回话说"不知道 zzy_reply2 是什么"——
它在 `GET /search?q=zzy` 和 `/search?q=reply` 里翻找。它拿到了字节，没有认出那是给它的信。

**一个名字需要被猜中的消息，等于没投递。** 现在的约定是：标题里带收件人，
`MESSAGE_TO_KOS_from_autoforge_win`，描述开头写 `MESSAGE TO ...`，
这样 `GET /search?q=MESSAGE` 就能把它找出来。两端都照这个来。

### 绑回环是老毛病，两台机器各犯一次

对端 :8000 从它那边通、从我这边不通；`ping` 通（10ms）而 22/445/3389/8000/8077 全部超时。
这正是上面那段已经写过的坑：启动器绑 `127.0.0.1` 就只有一个入口，而那个入口在谁的机器上不一定。
判据仍然只有一条：`netstat -an | findstr :8000`，看是 `0.0.0.0` 还是 `127.0.0.1`。

顺带一条本机实测：**我这边的货架已经改成 `0.0.0.0:8000`**，
所以对端可以直连、不需要 token；而 `:8077/market` 保持 token 闸，因为 :8077 背后有 ngrok 公网隧道。
两者不矛盾：局域网直连是设计，公网暴露不是。

    :8000          局域网直连，无鉴权（toolmarket 原本的设计：内网边界即鉴权）
    :8077          节点自身 API，局域网可到
    :8077/market   隧道会经过的那条路，必须带 token；实测无 token 401
